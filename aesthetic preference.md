...
- Clock and market widgets should behave like one stable desktop background layer on the main workspace and remain visibly attached to that workspace when Activities zooms it out. Put the non-interactive overview replica in the same full-monitor wallpaper background group so it receives the wallpaper's clipping and zoom transform; keep the replica at the monitor's fixed aspect ratio with one shared scale factor on both axes so the clock remains circular, and never reparent or independently shrink the live desktop actor through Shell's private overview actors.
