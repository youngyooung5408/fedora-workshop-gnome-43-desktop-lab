...
const DOCK_WIDTH = 92;
const DOCK_EDGE_REVEAL_WIDTH = 10;
...
export default class DesktopLabV12Extension extends Extension {
    ...
    _hideDock() {
        if (!this._dock || !this._dockRevealed || this._dockDrag)
            return;
    ...
    _addMarketSymbolFromEntry() {
        const symbol = this._sanitizeMarketSymbol(this._stockEntry?.get_text?.() ?? this._stockEntry?.clutter_text?.text);
        if (!symbol)
            return;
        ...
        if (typeof this._stockEntry?.set_text === 'function')
            this._stockEntry.set_text('');
        else if (this._stockEntry?.clutter_text)
            this._stockEntry.clutter_text.text = '';
