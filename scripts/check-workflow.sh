...
require_text "aesthetic preference.md" "Desktop clocks should stay very simple" "aesthetic preference records v1.2.4 clock preference"
require_text "aesthetic preference.md" "folder-style access to app clusters" "aesthetic preference records v1.2.4 app cluster preference"
require_text "TASK.md" 'Version `v1.2.4`' "TASK.md records v1.2.4 as the current request"
require_text "task/v 1.2/v 1.2.4.md" "## version 1.2.4" "v1.2.4 task note exists"
...
require_text "$tuned_v12_extension/extension.js" "DOCK_HOVER_SCALE" "v1.2.4 dock includes hover magnification"
require_text "$tuned_v12_extension/extension.js" "_buildFolderFlyout" "v1.2.4 dock includes app cluster folder flyouts"
require_text "$tuned_v12_extension/extension.js" "_showClusterFolder" "v1.2.4 app clusters can open folder flyouts"
require_text "$tuned_v12_extension/extension.js" "_drawClockFace" "desktop-lab-v12 includes a circular wall clock"
require_text "$tuned_v12_extension/extension.js" "const CLOCK_SIZE = 198" "v1.2.4 clock is 1.5 times larger than v1.2.3"
require_absent_text "$tuned_v12_extension/extension.js" "for (let tick = 0; tick < 60; tick++)" "v1.2.4 clock removes minute tick scale"
require_text "$tuned_v12_extension/extension.js" "now.format('%B %Y')" "v1.2.4 clock date shows only month and year"
require_text "$tuned_v12_extension/extension.js" "MARKET_SYMBOLS" "desktop-lab-v12 includes visible market symbols"
require_text "$tuned_v12_extension/extension.js" "Yahoo Chart API" "v1.2.4 market data uses Yahoo chart API"
...
require_text "$tuned_v12_extension/extension.js" "add_idle_watch" "v1.2.4 rest screen uses GNOME Shell idle monitor when available"
require_text "$tuned_v12_extension/extension.js" "_resetIdleTimer" "v1.2.4 rest screen keeps timer fallback"
...
require_text "$tuned_v12_metadata" "v1.2.4 refinement" "desktop-lab-v12 metadata records v1.2.4 refinement"
require_text "$tuned_v12_stylesheet" "background-gradient-direction: vertical;" "v1.2.4 uses textured translucent surfaces"
require_text "$tuned_v12_stylesheet" "desktop-lab-v12-folder-flyout" "v1.2.4 stylesheet includes app cluster folder flyout"
require_text "$tuned_v12_stylesheet" "font-weight: 600;" "v1.2.4 uses restrained component typography"
require_text "$tuned_v12_extension/extension.js" "text: 'Rest'" "v1.2.4 rest screen label is concise"
require_text "$tuned_v12_extension/extension.js" "0.96, 0.96, 0.97" "v1.2.4 drawing code uses a low-saturation neutral palette"
require_absent_text "$tuned_v12_stylesheet" "rgba(20, 119, 184" "v1.2.4 removes old saturated blue hover accent"
require_absent_text "$tuned_v12_stylesheet" "rgba(20, 184, 166" "v1.2.4 removes old saturated teal gesture accent"
require_absent_text "$tuned_v12_extension/extension.js" "0.24, 0.66, 0.96" "v1.2.4 removes old blue idle animation lines"
require_absent_text "$tuned_v12_extension/extension.js" "0.20, 0.83, 0.60" "v1.2.4 removes old green idle animation lines"

version_v124="versions/v1/v1.2/v1.2.4"
version_v124_extension="$version_v124/profile/extensions/desktop-lab-v12@young"
require_file "$version_v124/apply-v1.2.4.sh"
require_file "$version_v124/Apply v1.2.4.desktop"
require_file "$version_v124/profile/gsettings-export.sh"
require_text "$version_v124_extension/metadata.json" "v1.2.4 refinement" "v1.2.4 launcher snapshot includes v1.2.4 metadata"
require_text "$version_v124_extension/extension.js" "const CLOCK_SIZE = 198" "v1.2.4 launcher snapshot includes larger clock"
require_text "$version_v124_extension/extension.js" "Yahoo Chart API" "v1.2.4 launcher snapshot includes market API fetcher"
