...
- The v1.2.7 refinement keeps the clock and stock widgets on the main workspace background layer when the current Shell exposes it, adds overview-scale animation so they shrink with the background during Activities overview, smooths the hinted left-edge dock reveal, enlarges app flyouts, adds stock preset buttons, and adds a compact dock editor saved to the user config directory.
