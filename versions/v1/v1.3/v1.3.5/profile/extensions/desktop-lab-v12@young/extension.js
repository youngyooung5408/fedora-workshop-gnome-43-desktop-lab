...
const MARKET_SEARCH_DEBOUNCE_MS = 320;
const MAX_MARKET_INSTRUMENTS = 10;
...
const RUNNING_APP_HEIGHT = 54;
const DOCK_UTILITY_HEIGHT = 128;
...
export default class DesktopLabV12Extension extends Extension {
    enable() {
        ...
        this._marketSearchTimeoutId = 0;
        this._marketEditing = false;
        ...
        this._overviewWidgetClone = null;
        this._overviewWidgetCloneFrame = null;
        this._overviewWidgetCloneContainer = null;
        this._overviewWidgetCloneAllocationSignalId = 0;
        ...
        this._signals.push([
            this._appSystem,
            this._appSystem.connect('app-state-changed', () => this._rebuildRunningApps()),
        ]);
        this._signals.push([
            global.stage,
        ...
        this._signals.push([
            Main.overview,
            Main.overview.connect('shown', () => this._syncOverviewWidgetClone()),
        ]);
        this._signals.push([
            Main.overview,
            Main.overview.connect('hiding', () => this._hideStockChooser()),
        ]);
        this._signals.push([
            Main.overview,
            Main.overview.connect('hidden', () => {
                this._detachOverviewWidgetClone();
                this._syncWorkspaceVisibility();
            }),
        ]);
        this._signals.push([
            Main.overview,
            Main.overview.connect('window-drag-begin', () => this._cancelDockDrag()),
        ]);
    ...
    disable() {
        ...
        this._marketRowsScroll = null;
        this._marketEditButton = null;
        this._marketAddButton = null;
        this._stockChooser = null;
        this._stockEntry = null;
        this._stockSearchScroll = null;
        this._stockSearchResults = null;
        this._stockSearchStatus = null;
        this._twelveDataKeyEntry = null;
        this._twelveDataKeyRow = null;
        ...
        this._runningDockButtons = null;
        this._dockRunningApps = null;
        this._dockRunningSeparator = null;
        this._dockDrag = null;
        this._dockPinnedByCluster = false;
        this._focusedDockWindow = null;
    ...
    _rebuildDockGroups() {
        ...
        const editCluster = new St.BoxLayout({
            vertical: true,
            style_class: 'desktop-lab-v12-cluster desktop-lab-v12-dock-utility',
        });
        const editButton = new St.Button({
            reactive: true,
            can_focus: true,
            track_hover: true,
            accessible_name: 'Edit dock groups',
            style_class: 'desktop-lab-v12-app-button desktop-lab-v12-dock-edit-button',
        });
        editButton.set_pivot_point(0.5, 0.5);
        editButton.set_child(new St.Icon({
            icon_name: 'document-edit-symbolic',
            icon_size: 28,
        }));
        editButton.connect('clicked', () => this._toggleDockEditor());
        this._dockButtons.push(editButton);
        editCluster.add_child(editButton);
        this._dock.add_child(editCluster);

        const appsCluster = new St.BoxLayout({
            vertical: true,
            style_class: 'desktop-lab-v12-cluster desktop-lab-v12-dock-utility',
        });
        const appsButton = new St.Button({
            reactive: true,
            can_focus: true,
            track_hover: true,
            accessible_name: 'Show Applications',
            style_class: 'desktop-lab-v12-app-button desktop-lab-v12-show-apps-button',
        });
        appsButton.set_pivot_point(0.5, 0.5);
        appsButton.set_child(new St.Icon({
            icon_name: 'view-app-grid-symbolic',
            icon_size: 28,
        }));
        appsButton.connect('clicked', () => this._showApplications());
        this._dockButtons.push(appsButton);
        appsCluster.add_child(appsButton);
        this._dock.add_child(appsCluster);

        this._dockRunningSeparator = new St.Widget({
            visible: false,
            style_class: 'desktop-lab-v12-dock-separator',
        });
        this._dockRunningApps = new St.BoxLayout({
            vertical: true,
            visible: false,
            style_class: 'desktop-lab-v12-running-apps',
        });
        this._dock.add_child(this._dockRunningSeparator);
        this._dock.add_child(this._dockRunningApps);
        this._rebuildRunningApps();
        this._syncLayout();
    }

    _rebuildRunningApps() {
        if (!this._dockRunningApps)
            return;

        const oldButtons = new Set(this._runningDockButtons ?? []);
        this._dockButtons = (this._dockButtons ?? []).filter(button => !oldButtons.has(button));
        this._runningDockButtons = [];
        this._dockRunningApps.destroy_all_children();

        const seen = new Set();
        const runningApps = (this._appSystem?.get_running?.() ?? []).filter(app => {
            const id = app.get_id?.() ?? '';
            if (!id || seen.has(id))
                return false;
            seen.add(id);
        ...
        for (const app of runningApps)
            this._dockRunningApps.add_child(this._createRunningAppButton(app));

        const hasRunningApps = runningApps.length > 0;
        this._dockRunningSeparator.visible = hasRunningApps;
        this._dockRunningApps.visible = hasRunningApps;
        this._syncLayout();
    }

    _createRunningAppButton(app) {
        const button = new St.Button({
            reactive: true,
            can_focus: true,
            track_hover: true,
            accessible_name: app.get_name?.() ?? 'Running application',
            style_class: 'desktop-lab-v12-app-button desktop-lab-v12-running-app',
        });
        button.set_pivot_point(0.5, 0.5);
        button.set_child(app.create_icon_texture(DOCK_ICON_SIZE));
        button.connect('clicked', () => {
            app.activate();
            if (Main.overview?.visible)
                Main.overview.hide();
        });
        this._runningDockButtons.push(button);
    ...
    _buildMarketPanel() {
        ...
        this._marketEditButton = new St.Button({
            reactive: true,
            can_focus: true,
            track_hover: true,
            accessible_name: 'Edit market instruments',
            style_class: 'desktop-lab-v12-market-configure',
        });
        this._marketEditButton.set_child(new St.Icon({
            icon_name: 'document-edit-symbolic',
            icon_size: 14,
        }));
        this._marketEditButton.connect('clicked', () => this._toggleMarketEditing());
        header.add_child(this._marketEditButton);

        this._marketAddButton = new St.Button({
            reactive: true,
            can_focus: true,
            track_hover: true,
            visible: false,
            accessible_name: 'Add market instrument',
            style_class: 'desktop-lab-v12-market-configure',
        });
        this._marketAddButton.set_child(new St.Icon({
            icon_name: 'list-add-symbolic',
            icon_size: 14,
        }));
        this._marketAddButton.connect('clicked', () => this._toggleStockChooser());
        header.add_child(this._marketAddButton);
        ...
        this._marketRowsScroll = new St.ScrollView({
            overlay_scrollbars: true,
            clip_to_allocation: true,
            hscrollbar_policy: St.PolicyType.NEVER,
            vscrollbar_policy: St.PolicyType.AUTOMATIC,
            style_class: 'desktop-lab-v12-market-rows-scroll',
            child: this._marketRowsContainer,
        });
        this._marketPanel.add_child(this._marketRowsScroll);
    ...
    _buildStockChooser() {
        ...
        this._stockEntry.clutter_text.connect('text-changed', () => this._scheduleMarketSearch());
        const keyButton = this._createIconButton('dialog-password-symbolic', 'Set market API key', () => this._toggleMarketKeyEntry());
        keyButton.add_style_class_name('desktop-lab-v12-stock-icon-button');
        searchRow.add_child(this._stockEntry);
        searchRow.add_child(keyButton);

        this._twelveDataKeyRow = new St.BoxLayout({
            vertical: false,
            visible: false,
            style_class: 'desktop-lab-v12-stock-entry-row',
        });
        ...
        this._twelveDataKeyRow.add_child(this._twelveDataKeyEntry);
        this._twelveDataKeyRow.add_child(saveKeyButton);

        this._stockSearchStatus = new St.Label({text: '', style_class: 'desktop-lab-v12-editor-status'});
        this._stockSearchResults = new St.BoxLayout({vertical: true, style_class: 'desktop-lab-v12-stock-search-results'});
        this._stockSearchScroll = new St.ScrollView({
            overlay_scrollbars: true,
            clip_to_allocation: true,
            hscrollbar_policy: St.PolicyType.NEVER,
            vscrollbar_policy: St.PolicyType.AUTOMATIC,
            style_class: 'desktop-lab-v12-stock-search-scroll',
            child: this._stockSearchResults,
        });
        this._stockSearchScroll.set_height(184);

        this._stockChooser.add_child(searchRow);
        this._stockChooser.add_child(this._twelveDataKeyRow);
        this._stockChooser.add_child(this._stockSearchStatus);
        this._stockChooser.add_child(this._stockSearchScroll);
    ...
    _loadDockGroups() {
        ...
        try {
            ...
            const cleaned = groups
                ...
                .filter(group => group)
                .filter(group => group.actions.length > 0 || group.label.toLowerCase() !== 'apps');
    ...
    _loadMarketInstruments() {
        ...
        try {
            ...
            const seen = new Set();
            const unique = cleaned.filter(record => {
            ...
            // Older releases allowed an unbounded list. Keep the most recently
            // added instruments when reading that legacy configuration.
            return unique.slice(-MAX_MARKET_INSTRUMENTS);
        } catch (error) {
            return DEFAULT_MARKET_INSTRUMENTS.map(item => ({...item}));
        }
    }
    ...
    _rebuildMarketRows() {
        ...
        this._marketRowsScroll?.set_height(Math.min(
            MARKET_VISIBLE_ROWS,
            this._marketInstruments.length
        ) * MARKET_ROW_HEIGHT);
    }
    ...
    _showStockChooser() {
        ...
        if (this._marketInstruments.length >= MAX_MARKET_INSTRUMENTS)
            this._stockSearchStatus.text = `Limit reached · ${MAX_MARKET_INSTRUMENTS} instruments`;
    ...
    _positionStockChooser() {
        ...
        const chooserWidth = Math.max(1, Math.floor(marketWidth));
    ...
    _searchMarketInstruments() {
        if (this._marketSearchTimeoutId) {
            this._removeTimeout(this._marketSearchTimeoutId);
            this._marketSearchTimeoutId = 0;
        }
        const query = this._cleanDockText(this._getEntryText(this._stockEntry), 64);
        if (!query) {
            this._marketSearchGeneration += 1;
            this._stockSearchResults?.destroy_all_children();
            if (this._stockSearchStatus)
                this._stockSearchStatus.text = '';
            return;
        }
        const searchGeneration = ++this._marketSearchGeneration;
        this._stockSearchStatus.text = 'Searching…';
        this._stockSearchResults.destroy_all_children();
        this._runMarketHelper(['search', query], null, (payload, error) => {
            ...
            this._stockSearchStatus.text = payload.results.length ? '' : 'No matches';
    ...
    _scheduleMarketSearch() {
        ...
        this._marketSearchTimeoutId = this._addMillisecondTimeout(
            MARKET_SEARCH_DEBOUNCE_MS,
            () => {
                const timeoutId = this._marketSearchTimeoutId;
                this._marketSearchTimeoutId = 0;
                this._timeouts = this._timeouts.filter(id => id !== timeoutId);
                this._searchMarketInstruments();
    ...
    _createMarketSearchResult(instrument) {
        ...
        const label = new St.Label({
            text,
            x_expand: true,
            style_class: 'desktop-lab-v12-market-search-label',
        });
    ...
    _addMarketInstrument(value) {
        ...
        if (this._marketInstruments.length >= MAX_MARKET_INSTRUMENTS) {
            this._stockSearchStatus.text = `Remove an instrument before adding another · limit ${MAX_MARKET_INSTRUMENTS}`;
            return;
        }
    ...
    _removeMarketInstrument(instrument) {
        ...
        this._positionStockChooser();
        if (this._stockSearchStatus)
            this._stockSearchStatus.text = `${instrument.symbol} removed`;
        if (this._marketInstruments.length > 0)
            this._refreshMarketQuotes();
    }

    _toggleMarketEditing() {
        this._setMarketEditing(!this._marketEditing);
    }

    _setMarketEditing(editing) {
        this._marketEditing = Boolean(editing);
        this._marketEditButton?.set_child(new St.Icon({
            icon_name: this._marketEditing ? 'object-select-symbolic' : 'document-edit-symbolic',
            icon_size: 14,
        }));
        if (this._marketEditButton)
            this._marketEditButton.accessible_name = this._marketEditing
                ? 'Finish editing market instruments'
                : 'Edit market instruments';
        if (this._marketAddButton)
            this._marketAddButton.visible = this._marketEditing;
        if (this._marketEditing)
            this._marketPanel?.add_style_pseudo_class('editing');
        else {
            this._marketPanel?.remove_style_pseudo_class('editing');
            this._hideStockChooser();
        }
        this._rebuildMarketRows();
    }

    _toggleMarketKeyEntry() {
        if (!this._twelveDataKeyRow)
            return;
        this._twelveDataKeyRow.visible = !this._twelveDataKeyRow.visible;
        if (this._twelveDataKeyRow.visible)
            this._twelveDataKeyEntry?.grab_key_focus();
        this._positionStockChooser();
    }
    ...
    _saveTwelveDataKey() {
        ...
        if (!key) {
            this._stockSearchStatus.text = 'Key required';
            return;
        }
        this._runMarketHelper(['set-key'], key, (_payload, error) => {
            this._setEntryText(this._twelveDataKeyEntry, '');
            this._stockSearchStatus.text = error || 'Key saved';
            if (!error) {
                this._twelveDataKeyRow.visible = false;
                this._refreshMarketQuotes();
                this._searchMarketInstruments();
            }
        });
    ...
    _getOverviewWorkspaceBackgroundActor() {
        ...
        const wallpaperActor = background?._bgManager?.backgroundActor ?? null;
        if (wallpaperActor?.get_parent?.() !== background?._backgroundGroup)
            return null;

        // A sibling added to _backgroundGroup participates in its BinLayout
        // and can change only this workspace's requested geometry. Parenting
        // the replica under the actual wallpaper actor makes it follow the
        // wallpaper without affecting workspace allocation.
        return wallpaperActor;
    }

    _syncOverviewWidgetClone() {
        if (!Main.overview?.visible || !this._backgroundWidgetLayer) {
            this._detachOverviewWidgetClone();
            return;
        }

        const wallpaperActor = this._getOverviewWorkspaceBackgroundActor();
        if (!wallpaperActor) {
            this._detachOverviewWidgetClone();
            return;
        }

        if (this._overviewWidgetCloneFrame?.get_parent?.() === wallpaperActor) {
            this._syncOverviewWidgetCloneGeometry();
            return;
        }
        ...
        const clone = new Clutter.Clone({
            source: this._backgroundWidgetLayer,
            reactive: false,
        });
        ...
        const allocationSignalId = wallpaperActor.connect('notify::allocation', () => {
            if (this._overviewWidgetCloneFrame === frame)
                this._syncOverviewWidgetCloneGeometry();
        });
        ...
        wallpaperActor.add_child(frame);
        frame.raise_top?.();
        this._overviewWidgetClone = clone;
        this._overviewWidgetCloneFrame = frame;
        this._overviewWidgetCloneContainer = wallpaperActor;
        this._overviewWidgetCloneAllocationSignalId = allocationSignalId;
        this._syncOverviewWidgetCloneGeometry();
    }

    _syncOverviewWidgetCloneGeometry() {
        const frame = this._overviewWidgetCloneFrame;
        const clone = this._overviewWidgetClone;
        const wallpaperActor = this._overviewWidgetCloneContainer;
        const source = this._backgroundWidgetLayer;
        if (!frame || !clone || !wallpaperActor || !source)
            return;

        const [wallpaperWidth, wallpaperHeight] = wallpaperActor.get_size();
        ...
        frame.set_size(wallpaperWidth, wallpaperHeight);

        // Keep the clone at native monitor dimensions and apply one shared
        // transform inside the wallpaper actor. This makes both the main and
        // neighboring workspace previews use Shell's own wallpaper geometry.
        const scale = Math.min(
            wallpaperWidth / sourceWidth,
            wallpaperHeight / sourceHeight
        );
        clone.set_size(sourceWidth, sourceHeight);
        clone.set_scale(scale, scale);
        clone.set_position(
            Math.round((wallpaperWidth - sourceWidth * scale) / 2),
            Math.round((wallpaperHeight - sourceHeight * scale) / 2)
        );
    }

    _detachOverviewWidgetClone() {
        const frame = this._overviewWidgetCloneFrame;
        const clone = this._overviewWidgetClone;
        const container = this._overviewWidgetCloneContainer;
        const allocationSignalId = this._overviewWidgetCloneAllocationSignalId;
        this._overviewWidgetClone = null;
        this._overviewWidgetCloneFrame = null;
        this._overviewWidgetCloneContainer = null;
        this._overviewWidgetCloneAllocationSignalId = 0;
        if (!frame && !clone)
            return;
        try {
            if (container && allocationSignalId)
                container.disconnect(allocationSignalId);
            if (frame)
                frame.destroy();
            else
                clone.destroy();
        } catch (error) {
            // The overview may already have destroyed its workspace actors.
        }
    }
    ...
    _hideOverviewDash() {
        ...
        if (!dash)
            return;
        if (this._overviewDash !== dash) {
            this._overviewDash = dash;
            this._overviewDashState = {
                visible: dash.visible,
                reactive: dash.reactive,
                opacity: dash.opacity,
            };
        }
        dash.add_style_class_name('desktop-lab-v12-hidden-overview-dash');
        dash.reactive = false;
        dash.opacity = 0;
        dash.hide();
    }
    ...
    _restoreOverviewDash() {
        const dash = this._overviewDash;
        const state = this._overviewDashState;
        this._overviewDash = null;
        this._overviewDashState = null;
        if (!dash || !state)
            return;
        try {
            dash.remove_style_class_name('desktop-lab-v12-hidden-overview-dash');
            dash.reactive = state.reactive;
            dash.opacity = state.opacity;
            if (state.visible)
                dash.show();
            else
                dash.hide();
        } catch (error) {
            // Shell can replace overview actors while the session is ending.
        }
    }
    ...
    _syncLayout() {
        ...
        const runningCount = this._runningDockButtons?.length ?? 0;
        const runningHeight = runningCount > 0
            ? runningCount * RUNNING_APP_HEIGHT + 12
            : 0;
        const desiredDockHeight = clusterCount * DOCK_CLUSTER_HEIGHT +
            DOCK_UTILITY_HEIGHT + runningHeight + DOCK_VERTICAL_PADDING;
        ...
        const marketWidth = Math.max(298, Math.min(365, monitor.width - 220));
    ...
    _refreshMarketQuotes() {
        if (this._marketFetchInFlight || !this._quoteLabels || !this._enabled ||
            this._marketInstruments.length === 0)
            return;
        ...
        this._runMarketHelper(['quotes', JSON.stringify(this._marketInstruments)], null, (payload, error) => {
            ...
            for (const quote of payload.quotes ?? []) {
                ...
                if (label && quote.price) {
                    const currency = instrument.currency ? ` ${instrument.currency}` : '';
                    label.text = `${quote.price}${currency}${quote.cached ? ' ·' : ''}`;
                } else if (label && quote.error) {
                    label.text = quote.error.includes('API key')
                        ? 'Key needed'
                        : 'Unavailable';
                }
            }
    ...
    _dismissEditorsOutside(x, y) {
        if (this._dockEditor?.visible &&
            !this._pointInsideActor(this._dockEditor, x, y) &&
            !this._pointInsideActor(this._dock, x, y))
            this._hideDockEditor();

        if ((this._marketEditing || this._stockChooser?.visible) &&
            !this._pointInsideActor(this._marketPanel, x, y) &&
            !this._pointInsideActor(this._stockChooser, x, y))
            this._setMarketEditing(false);
    }
    ...
    _handleCapturedEvent(event) {
        ...
        if (type === Clutter.EventType.MOTION) {
            ...
            const [x, y] = event.get_coords();
            this._dismissEditorsOutside(x, y);
