...
- The v1.2.6 refinement places the clock and stock widgets on the main workspace background layer when the current Shell exposes it, hides those widgets on other workspaces, removes the clock second hand, adds 12 hourly clock dots, hides the left dock until the pointer reaches the left edge, simplifies the dock and app flyouts to icons without nested labeled boxes, and adds a configurable stock symbol chooser with optional Alpha Vantage key support and no-key fallback quotes.
