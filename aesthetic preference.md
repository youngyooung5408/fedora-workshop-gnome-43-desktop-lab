...
- Clock and market widgets should behave like one stable desktop background layer on the main workspace and remain visibly attached to that workspace when Activities zooms it out. Put the non-interactive overview replica under the actual full-monitor wallpaper actor, not beside it as another workspace-layout child, so it follows the wallpaper without changing that workspace's requested geometry. Keep the replica at the monitor's fixed aspect ratio with one shared scale factor on both axes, exclude it from Shell drag picking, and never reparent or independently shrink the live desktop actor through Shell's private overview actors.
...
- The GNOME overview should not add a second bottom dock. Keep the custom left dock as the single launcher surface, retain one compact Show Applications utility there, and append currently running applications to its bottom without a nested scrollbar.
...
- Data panels, including market panels, should remain visible and visually textured with restrained translucent surfaces, but keep only essential values visible and hide update-time or provider/API labels unless explicitly requested. Market panels should show the latest available amount with currency, cap watchlists at about 10 instruments, and scroll a fixed row area. Market editing should begin from a pen, expose direct × removal and an edit-only plus button, search automatically while typing, and keep result growth inside a fixed scroll area. Keep the chooser the same width as the main panel. Market chooser results should be verified instruments with clear name, exchange, and currency metadata; API setup should stay collapsed behind a compact key control.
- Editors and add panels should dismiss on a click in a plain area outside the active panel, matching the forgiving outside-click behavior of app-cluster flyouts.
