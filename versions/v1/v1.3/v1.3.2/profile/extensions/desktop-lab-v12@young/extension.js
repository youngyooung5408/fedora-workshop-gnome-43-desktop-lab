...
const RUNNING_APP_MAX_HEIGHT = 180;
const DOCK_UTILITY_HEIGHT = 64;
...
export default class DesktopLabV12Extension extends Extension {
    ...
    _rebuildDockGroups() {
        ...
        this._dockRunningScroll = new St.ScrollView({
            visible: false,
            overlay_scrollbars: true,
            clip_to_allocation: true,
            hscrollbar_policy: St.PolicyType.NEVER,
            vscrollbar_policy: St.PolicyType.AUTOMATIC,
            style_class: 'desktop-lab-v12-running-apps-scroll',
            child: this._dockRunningApps,
        });
        this._dock.add_child(this._dockRunningSeparator);
        this._dock.add_child(this._dockRunningScroll);
        this._rebuildRunningApps();
        this._syncLayout();
    }

    _rebuildRunningApps() {
        if (!this._dockRunningApps || !this._dockRunningScroll)
            return;
        ...
        this._dockRunningScroll.visible = hasRunningApps;
        this._dockRunningScroll.set_height(Math.min(
            RUNNING_APP_MAX_HEIGHT,
            Math.max(1, runningApps.length * RUNNING_APP_HEIGHT)
        ));
        this._syncLayout();
    }
    ...
    _toggleMarketEditing() {
        this._marketEditing = !this._marketEditing;
        this._marketEditButton?.set_child(new St.Icon({
    ...
    _syncOverviewWidgetClone() {
        ...
        const container = this._getOverviewWorkspaceBackgroundBin();
        if (!container) {
            this._detachOverviewWidgetClone();
            return;
        }
    ...
    _syncLayout() {
        ...
        const runningHeight = runningCount > 0
            ? Math.min(RUNNING_APP_MAX_HEIGHT, runningCount * RUNNING_APP_HEIGHT) + 12
            : 0;
