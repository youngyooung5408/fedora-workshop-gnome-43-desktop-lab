...
const DOCK_WIDTH = 92;
const DOCK_EDGE_REVEAL_WIDTH = 26;
const DOCK_HINT_WIDTH = 9;
const DOCK_HINT_REFRESH_MS = 1400;
...
const OVERVIEW_WIDGET_SCALE = 0.78;
const OVERVIEW_WIDGET_TRANSLATION_Y = 54;
...
export default class DesktopLabV12Extension extends Extension {
    enable() {
        ...
        this._signals.push([
            Main.overview,
            Main.overview.connect('showing', () => this._setOverviewWidgetMode(true)),
        ]);
        this._signals.push([
            Main.overview,
            Main.overview.connect('shown', () => this._setOverviewWidgetMode(true, false)),
        ]);
        this._signals.push([
            Main.overview,
            Main.overview.connect('hiding', () => this._setOverviewWidgetMode(false)),
        ]);
        this._signals.push([
            Main.overview,
            Main.overview.connect('hidden', () => this._setOverviewWidgetMode(false, false)),
        ]);
        ...
        this._addMillisecondTimeout(DOCK_HINT_REFRESH_MS, () => {
            this._dockHintPhase += 1;
            this._dockHint?.queue_repaint();
            return GLib.SOURCE_CONTINUE;
        });
    ...
    disable() {
        ...
        this._destroyChrome(this._dockHint);
        this._destroyChrome(this._dockEditor);
        this._destroyDesktopActor(this._clockPanel);
        this._destroyDesktopActor(this._marketPanel);
    ...
    _buildDock() {
        this._dockHint = new St.DrawingArea({
            reactive: false,
            opacity: 170,
            style_class: 'desktop-lab-v12-dock-hint',
        });
        this._dockHint.connect('repaint', area => this._drawDockHint(area));
        ...
        Main.layoutManager.addChrome(this._dockHint, {
            affectsStruts: false,
            trackFullscreen: false,
        });
    ...
    _buildClock() {
        ...
        this._addDesktopActor(this._clockPanel);
    }
    ...
    _buildMarketPanel() {
        ...
        this._addDesktopActor(this._marketPanel);
    }
    ...
    _showDock() {
        if (!this._dock || this._dockRevealed)
            return;
    ...
    _handleDockRevealMotion(x, y) {
        ...
        const panelHeight = Main.panel?.height ?? 0;
        const contentTop = monitor.y + panelHeight;
        if (x <= monitor.x + DOCK_EDGE_REVEAL_WIDTH && y >= contentTop) {
            this._showDock();
            return;
        }
    ...
    _setOverviewWidgetMode(inOverview, animate = true) {
        this._overviewWidgetsScaled = !!inOverview;
        if (inOverview)
            this._hideStockChooser();

        for (const actor of [this._clockPanel, this._marketPanel])
            this._setBackgroundWidgetTransform(actor, this._overviewWidgetsScaled, animate);
    }
    ...
    _setBackgroundWidgetTransform(actor, inOverview, animate) {
        ...
        const scale = inOverview ? OVERVIEW_WIDGET_SCALE : 1;
        const translationY = inOverview ? OVERVIEW_WIDGET_TRANSLATION_Y : 0;
    ...
    _syncLayout() {
        ...
        this._dockHiddenX = monitor.x - DOCK_WIDTH - 18;
        this._dock?.set_position(this._dockRevealed ? this._dockShownX : this._dockHiddenX, dockY);
        this._dock?.set_size(DOCK_WIDTH, dockHeight);
        this._dockHint?.set_position(monitor.x, contentTop);
        this._dockHint?.set_size(DOCK_HINT_WIDTH, availableHeight);
        this._dockRevealZone?.set_position(monitor.x, contentTop);
        this._dockRevealZone?.set_size(DOCK_EDGE_REVEAL_WIDTH, availableHeight);
        ...
        this._positionStockChooser();
        this._positionDockEditor();
        this._setOverviewWidgetMode(this._overviewWidgetsScaled, false);
    ...
    _drawDockHint(area) {
        ...
        try {
            const phase = this._dockHintPhase;
            const gradient = new Cairo.LinearGradient(0, 0, width, 0);
            gradient.addColorStopRGBA(0, 0.22, 0.58, 0.74, 0.42);
            gradient.addColorStopRGBA(1, 0.88, 0.96, 1.0, 0.0);
            cr.setSource(gradient);
            cr.rectangle(0, 0, width, height);
            cr.fill();

            cr.setSourceRGBA(0.84, 0.96, 1.0, 0.34);
            for (let index = 0; index < 14; index++) {
                const y = ((index * 73) + (phase * 19)) % Math.max(1, height);
                const radius = 0.8 + (index % 3) * 0.45;
                cr.arc(width * 0.42, y, radius, 0, TAU);
                cr.fill();
            }
        } finally {
            cr.$dispose();
