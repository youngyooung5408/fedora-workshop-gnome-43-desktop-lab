...
export default class DesktopLabV12Extension extends Extension {
    ...
    _getOverviewWorkspaceBackgroundGroup() {
        ...
        if (workspace?.metaWorkspace?.index?.() !== BACKGROUND_WORKSPACE_INDEX)
            return null;
        // Shell allocates _bin to the work area, but allocates
        // _backgroundGroup to the full monitor (including panel offsets).
        // Sharing the wallpaper group keeps this replica on the wallpaper's
        // exact aspect ratio, clipping path, and Activities transform.
        return workspace?._background?._backgroundGroup ?? null;
    }
    ...
    _syncOverviewWidgetClone() {
        ...
        const container = this._getOverviewWorkspaceBackgroundGroup();
        if (!container) {
            this._detachOverviewWidgetClone();
            return;
        }

        if (this._overviewWidgetCloneFrame?.get_parent?.() === container) {
        ...
        this._overviewWidgetCloneContainer = container;
        this._syncOverviewWidgetCloneGeometry();
    }
    ...
    _syncOverviewWidgetCloneGeometry() {
        ...
        if (!frame || !clone || !source)
            return;

        const [frameWidth, frameHeight] = frame.get_size();
        const [sourceWidth, sourceHeight] = source.get_size();
        if (frameWidth <= 0 || frameHeight <= 0 ||
            sourceWidth <= 0 || sourceHeight <= 0)
            return;

        // Clutter.Clone normally stretches its source to fill its allocation
        // with independent X/Y factors. Keep its native monitor allocation and
        // apply one shared transform so the clock remains circular while the
        // overview wallpaper moves between window-picker and app-grid states.
        ...
        clone.set_position(
            Math.round((frameWidth - sourceWidth * scale) / 2),
            Math.round((frameHeight - sourceHeight * scale) / 2)
        );
    }
