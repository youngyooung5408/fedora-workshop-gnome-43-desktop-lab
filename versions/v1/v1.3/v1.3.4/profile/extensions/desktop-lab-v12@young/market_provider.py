...
USER_AGENT = "desktop-lab-v1.3.3/1.0"
...
def parse_taiwan_realtime(payload: dict[str, Any]) -> dict[tuple[str, str], str]:
    if str(payload.get("rtcode", "0000")) != "0000":
        raise ProviderError(str(payload.get("rtmessage") or "Taiwan live quotes unavailable"))

    prices: dict[tuple[str, str], str] = {}
    for row in payload.get("msgArray") or []:
        provider = {"tse": "twse", "otc": "tpex"}.get(str(row.get("ex") or "").strip())
        symbol = str(row.get("c") or "").strip()
        price = _clean_price(row.get("z")) or _clean_price(row.get("pz"))
        if provider and symbol and price:
            prices[(provider, symbol)] = price
    return prices
...
def parse_twelve_price(payload: dict[str, Any]) -> str:
    if payload.get("status") == "error" or payload.get("code"):
        raise ProviderError(str(payload.get("message") or "Twelve Data price failed"))
    price = _clean_price(payload.get("price"))
    if not price:
        raise ProviderError("Twelve Data returned no latest price")
    return price
...
def _taiwan_latest_prices(records: list[dict[str, Any]]) -> dict[tuple[str, str], str]:
    channels = []
    for record in records:
        provider = str(record.get("provider") or "")
        symbol = str(record.get("symbol") or "").strip()
        if provider not in {"twse", "tpex"} or not symbol:
            continue
        prefix = "tse" if provider == "twse" else "otc"
        channels.append(f"{prefix}_{symbol}.tw")
    if not channels:
        return {}

    query = urlencode({
        "ex_ch": "|".join(dict.fromkeys(channels)),
        "json": "1",
        "delay": "0",
    })
    ...
    return parse_taiwan_realtime(payload)
...
def quotes(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    try:
        taiwan_latest = _taiwan_latest_prices(records)
    ...
    for original in records:
        ...
        try:
            ...
            if provider in {"twse", "tpex"}:
                ...
                price = taiwan_latest.get((provider, symbol)) or item.get("close")
                ...
                payload = _request_json(
                    f"{TWELVE_BASE}/price?{urlencode(params)}",
                    {"Authorization": f"apikey {api_key}"},
                )
                price = parse_twelve_price(payload)
            elif provider == "legacy":
                if not api_key:
                    raise ProviderError("Twelve Data API key required")
                payload = _request_json(
                    f"{TWELVE_BASE}/quote?{urlencode({'symbol': symbol})}",
                    {"Authorization": f"apikey {api_key}"},
                )
            ...
            if not price:
                raise ProviderError("Latest price unavailable")
            cache[cache_key] = {"price": price, "updated": int(time.time())}
        except ProviderError as provider_error:
            error = str(provider_error)
            cached = cache.get(cache_key) or {}
            price = cached.get("price") or cached.get("close")
    ...
    if any(item.get("price") and not item.get("cached") for item in output):
        _save_cache(cache)
