...
- Clock and market widgets should behave like one stable desktop background layer on the main workspace and remain visibly attached to that workspace when Activities zooms it out. Use a non-interactive overview replica that follows the workspace background; never reparent or independently shrink the live desktop actor through Shell's private overview actors.
...
- The GNOME overview should not add a second bottom dock. Keep the custom left dock as the single launcher surface, remove its Show Applications shortcut, and append currently running applications to its bottom in a bounded scroll area.
