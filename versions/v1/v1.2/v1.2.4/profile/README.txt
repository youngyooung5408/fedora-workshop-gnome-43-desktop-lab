...
- The desktop-lab-v12@young extension adds a larger Mac-like middle-left grouped app dock, a simplified large wall-clock overlay, a visible API-backed market board, a bottom drag/scroll app-grid zone, and a 30-minute animated rest screen.
- The v1.2.4 refinement removes clock minute tick marks, shows only month and year on the clock date label, adds hover magnification and folder flyouts for app clusters, switches market data to the Yahoo chart API, and uses the GNOME Shell idle monitor when available.
